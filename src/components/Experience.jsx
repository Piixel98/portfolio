    ...
    <section id="experience" className="px-[5%] py-24">
      ...
      <div ref={sectionRef} className="relative pl-8 max-w-3xl">
        ...
        <div
          ...
          style={{ transition: 'height 1.4s ease' }}
        />

        {experience.jobs.map((job, i) => (
          <div key={`${job.company}-${job.dates}`} className="js-fade relative mb-14 group" style={{ transitionDelay: `${i * 0.15}s` }}>
            ...
            <div className="font-mono text-[11px] text-emerald-400 tracking-[0.1em] mb-1.5">{job.dates}</div>
            ...
            <ul className="text-[14px] text-[#5A6478] leading-[1.75] space-y-1.5 mb-5">
              {job.bullets.map(bullet => (
                <li key={bullet} className="pl-4 relative before:content-['-'] before:absolute before:left-0 before:text-blue-400">
                  {bullet}
                </li>
              ))}
            </ul>
            ...
            <div className="flex flex-wrap gap-1.5">
              {job.stack.map(tag => <span key={tag} className="tl-tag">{tag}</span>)}
            </div>
          </div>
        ))}
      </div>
